...
d3.json("data/tcgaCancerGraphs.json", function(cancerCodes) {
	for(i in cancerCodes) {
		cancerCodeList[cancerCodes[i].id] = {"methNode" : cancerCodes[i].methNode, "exonNode" : cancerCodes[i].exonNode, "patients": []};
	}
...
function retrievePatientList(tumorChecked, counter) {
	...
	var myInterval = setInterval(function(){
		...
		//	console.log(patients);
			d3.select("#tcgaBar").append("select").attr("id", "patientList").attr("multiple", "multiple").selectAll("option").data(patients).enter().append("option").text(function(d) {return d.patientCode; }).attr("value", function(d){ return d.patientCode;});
